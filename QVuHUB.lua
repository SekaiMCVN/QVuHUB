...
local function Corner(parent, props)
    local new = Create("UICorner", parent)
        new.CornerRadius = QVuHUB.Corner_Radius
        if props then
            SetProps(new, props)
        end
    return new
...
local function TextSetColor(instance)
    ...
    instance.MouseLeave:Connect(function()
        CreateTween(instance, "TextColor3", QVuHUB.Cor_Text, 0.4, false)
    end)
end

local ScreenGui = Create("ScreenGui", CoreGui, {
    Name = "QVuHUB library - Realm"
})
...
function MakeNotifi(Configs)
    local Title = Configs.Title or "QVu HUB - Realm"
    ...
    local Frame2 = Create("Frame", Frame1, {
        ...
        BackgroundColor3 = QVuHUB.Cor_Hub,
    ...
    local TextLabel = Create("TextLabel", Frame2, {
        ...
        Font = QVuHUB.Text_Font,
        BackgroundTransparency = 1,
        Text = Title,
        TextSize = 20,
        Position = UDim2.new(0, 20, 0, 5),
        TextXAlignment = "Left",
        TextColor3 = QVuHUB.Cor_Text
    })
    ...
    local TextLabel = Create("TextLabel", Frame2, {
        ...
        TextSize = 15,
        TextColor3 = QVuHUB.Cor_DarkText,
        TextXAlignment = "Left",
        TextYAlignment = "Top",
        AutomaticSize = "Y",
        Text = text,
    ...
    local FrameSize = Create("Frame", Frame2, {
        ...
        BackgroundColor3 = QVuHUB.Cor_Stroke,
...
function MakeWindow(Configs)
    local title = Configs.Hub.Title or "QVu HUB - Realm"
    local Anim_Title = Configs.Hub.Animation or "by : SekaiMCVN"
    ...
    local KeyKey = Configs.Key.Keys or {"FreeConMeMay"}
    ...
    local KeyCopyKeyLink = Configs.Key.Notifi.CopyKeyLink or "Đã sao chép vào Bảng tạm"
    ...
    if KeySystem then
        ...
        local ConfirmButton = Create("TextButton", KeyMenu, {
            ...
            BackgroundColor3 = QVuHUB.Cor_Options
    ...
    local function CreateClose()
        ...
        local Mensage = Create("TextLabel", CloseMenu, {
            ...
            Text = "bạn có chắc chắn muốn đóng script này không??",
    ...
    function MinimizeButton(Configs)
        ...
        local color = Configs.Color or QVuHUB.Cor_Hub
        local corner = Configs.Corner or true
        local stroke = Configs.Stroke or false
        local strokecolor = Configs.StrokeColor or QVuHUB.Cor_Stroke
        ...
        if stroke then
            Stroke(Button, {Color = strokecolor})
        end
    ...
    local function Add_Line(props)
        ...
        line.BackgroundColor3 = QVuHUB.Cor_Stroke
    ...
    local FirstTab = true
    local textsize = 15
    local textcolor = QVuHUB.Cor_Text
    ...
    function MakeTab(Configs)
        ...
        if TabTitle then
            Create("TextLabel",Container,{BackgroundTransparency=1,Text="#"..string.gsub(TabName," ","-"),TextSize=25,Font=QVuHUB.Text_Font,TextXAlignment="Left",TextColor3=QVuHUB.Cor_Text,Size=UDim2.new(1, 0, 0, 30),Position=UDim2.new(0, 30, 0, 0),Name="Frame"})
        end
        ...
        TextButton.MouseButton1Click:Connect(function()
            ...
            for _,frame in pairs(ScrollBar:GetChildren()) do
                if frame:IsA("Frame") and frame:FindFirstChild("TextLabel") and frame.TextLabel ~= TextLabel then
                    CreateTween(frame.TextLabel, "TextColor3", QVuHUB.Cor_DarkText, 0.3, false)
                    frame.TextLabel.TextSize = 14
                end
            end
            Container.Visible = true
            CreateTween(TextLabel, "TextColor3", QVuHUB.Cor_Text, 0.3, false)
    ...
    function AddButton(parent, Configs)
        ...
        local ImageLabel = Create("ImageLabel", TextButton, {
            ...
            BackgroundTransparency = 1,
            ImageColor3 = QVuHUB.Cor_Stroke
        })
    
        TextButton.MouseButton1Click:Connect(function()
            Callback("Click!!")
            CreateTween(ImageLabel, "ImageColor3", Color3.fromRGB(28, 120, 212), 0.2, true)
            CreateTween(ImageLabel, "ImageColor3", QVuHUB.Cor_Stroke, 0.2, false)
        end)
    ...
    function AddToggle(parent, Configs)
        ...
        TextButton.MouseButton1Click:Connect(function()
            if Frame2.Position.X.Offset < 5 then
                ...
                CreateTween(Frame2, "BackgroundColor3", QVuHUB.Cor_Stroke, 0.2, false)
                CreateTween(Stroke, "Color", QVuHUB.Cor_Stroke, 0.2, false)
                CreateTween(TextLabel, "TextColor3", QVuHUB.Cor_Text, 0.2, false)
                Callback(false)
            end
        end)
    ...
    function AddSlider(parent, Configs)
        ...
        local Slider = Create("Frame", SliderBar1, {
            BackgroundColor3 = QVuHUB.Cor_Text,
...
function AddColorPicker(parent, Configs)
    ...
    local SavePos2 = Create("Frame", grade, {
        Visible = false, Size = UDim2.new(1, 0, 0, 0)
    })
    ...
    click.MouseButton1Click:Connect(function()
        onoff = not onoff
        if onoff == true then
            local tween = TweenService:Create(TextButton, TweenInfo.new(0.2, Enum.EasingStyle.Linear), {Size = UDim2.new(1, 0, 0, 140)})tween:play()tween.Completed:Wait()
        else
            local tween = TweenService:Create(TextButton, TweenInfo.new(0.2, Enum.EasingStyle.Linear), {Size = UDim2.new(1, 0, 0, 25)})tween:play()tween.Completed:Wait()
        end
    end)
end
...
function AddDropdown(parent, Configs)
    ...
    local function AddOption(OptionName)
        ...
        if OptionName == Default then
            ...
            TextButton.TextColor3 = QVuHUB.Cor_Text
...
function AddDiscord(parent, Configs)
    ...
    JoinButton.MouseButton1Click:Connect(function()
        if ClickConter == 0 or tick() - time > 5 then time = tick() setclipboard(DiscordLink) ClickConter = ClickConter + 1
            JoinButton.Text = "Đã sao chép vào Bảng tạm"
